...
class Tournament {
	...
	onTournamentEnd() {
		...
		if (global.Shop) {
			let data = {results: this.generator.getResults().map(usersToNames), bracketData: this.getBracketData()}; data = data['results'].toString(); let runnerUp = false; let winner = false; if (data.indexOf(',') >= 0) {data = data.split(','); winner = data[0]; if (this.room.id === 'casino' && Users.get(Bot.config.name)) Chat.parse('/endtourbets ' + winner, this.room, Users.get(Bot.config.name), Users.get(Bot.config.name).connections[0]); if (data[1]) runnerUp = data[1];} else {winner = data;} let tourSize = this.generator.users.size; if (this.room.isOfficial && tourSize >= 3) {let firstMoney = tourSize * 20; let secondMoney = Math.floor((tourSize * 20) / 2); let firstBuck = 'Buck'; let secondBuck = 'Buck'; if (firstMoney > 1) firstBuck = 'Bucks'; if (secondMoney > 1) secondBuck = 'Bucks'; Shop.giveMoney(winner, firstMoney); this.room.add('|raw|<strong>' + Chat.escapeHTML(winner) + '</strong> has won ' + firstMoney + ' ' + firstBuck + ' for winning the tournament!'); if (runnerUp) { Shop.giveMoney(runnerUp, secondMoney); this.room.add('|raw|<strong>' + Chat.escapeHTML(runnerUp) + '</strong> has also won ' + secondMoney + ' ' + secondBuck + ' for coming in second place!');}}
		}
		this.isEnded = true;
		if (this.autoDisqualifyTimer) clearTimeout(this.autoDisqualifyTimer);
