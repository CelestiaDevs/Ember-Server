...
exports.commands = {
	...
	buypack: function (target, room, user) {
		...
		let amount = Shop.getUserMoney(user.userid);
		...
		Shop.removeMoney(user.userid, Number(-cost));
	...
	openpack: function (target, room, user) {
		...
		for (let i = 0; i < 3; i++) {
			...
			this.sendReplyBox(Plugins.Colors.apply(user.name, true) + ' got <font color="' + colors[cards[card].rarity] + '">' + cards[card].rarity + '</font> ' +
	...
	showcase: function (target, room, user) {
		...
		if (!cards.length || userid === "constructor") return this.sendReplyBox(Plugins.Colors.apply(userid, false) + " has no cards.");
	...
	tradecard: function (target, room, user) {
		...
		if (Users.get(targetUser)) Users.get(targetUser).send("|pm|~Card Shop [Do not Reply]|" + targetUser + "|/raw <div class=\"broadcast-green\">" + Plugins.Colors.apply(user.name, true) + " has initiated a trade with you.  Click <button name=\"send\" value=\"/trades last\">here</button> or use <b>/trades</b> to view your pending trade requests.</div>");
		user.send("|pm|~Card Shop [Do not Reply]|" + user.userid + "|/raw <div class=\"broadcast-green\">Your trade with " + Plugins.Colors.apply(targetUser, true) + " has been initiated.  Click <button name=\"send\" value=\"/trades last\">here</button> or use <b>/trades</b> to view your pending trade requests.</div>");
	},
	...
	viewcardtrades: function (target, room, user) {
		...
		let userSideDisplay = '<center>' + Plugins.Colors.apply(user.userid, true) + '<br />' + cardImage + "<br />" + cardRarityPoints + '</center>';
		...
		let targetSideDisplay = "<center>" + (displayTrade.from !== user.userid ? Plugins.Colors.apply(displayTrade.from, true) : Plugins.Colors.apply(displayTrade.to, true)) + '<br />' + cardImage + "<br />" + cardRarityPoints + "</center>";
	...
	tradeaction: function (target, room, user) {
		...
		case 'accept':
			...
			if (targetUsers[0]) {
				targetUsers[0].popup("|html|" + backButton + "<center>Your trade with " + Plugins.Colors.apply(trade.from, true) + " has gone through." +
				"<br /><button name=\"send\" value=\"/cs card, " + trade.fromExchange + "\">View Traded Card</button></center>"); // show card
			}
			if (targetUsers[1]) {
				targetUsers[1].popup("|html|<center>Your trade with " + Plugins.Colors.apply(trade.to, true) + " has gone through." +
